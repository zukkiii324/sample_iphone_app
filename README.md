...
このリポジトリは静的サイトのため本来のビルド工程はありませんが、
CIや自動チェックで `npm run build` が呼ばれたときに失敗しないよう、
以下の事前チェックを実行できるようにしています。
...
- HTML内のローカルリンク切れ（`href` / `src`）

問題がある場合はエラーで終了し、問題がなければ成功メッセージを表示します。
